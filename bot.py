...
class NPAMonitorBot:
    """Telegram bot for monitoring NPA Depot Manager records"""
...
🔗 **Database:** {'Connected' if self.table_generator.connect_to_database() else 'Failed'}
    ...
    async def manual_check_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for table_name in self.table_generator.table_names:
                ...
                try:
                    with self.table_generator.engine.connect() as conn:
                        query = text(f"""
                            SELECT order_date, order_number, products, volume, ex_ref_price, brv_number, bdc
                            FROM {text(table_name.lower()).compile(bind=self.table_generator.engine).string}
                            WHERE brv_number = :brv_number
                        """)
                        result = conn.execute(query, {"brv_number": brv_number}).fetchall()
                        for row in result:
            ...
            for idx, record in enumerate(found_records, start=1):
                row = record['data']
                message += (
                    f"**Record {idx} (Status: {record['table']})**\n"
...
• Database: {'Connected' if self.table_generator.connect_to_database() else 'Failed'}
    ...
    async def _check_for_new_records(self) -> int:
        """Check for new records and notify groups"""
        try:
            df, error = self.data_fetcher.fetch_data()
            if error:
...
"""
    ...
    async def run(self):
        ...
        try:
            ...
            webhook_url = f"https://https://report-bot-cryi.onrender.com/{self.bot_token}"
